...
import { getTabSkeletonHtml } from '/src/vault.js';
...
class FantasyApp {
    ...
    async init() {
        ...
        // Seamlessly preload all remaining tabs in the background during idle time (desktop only)
    ...
    async loadData() {
        ...
        // Fast critical path fetch (<300KB) - never block on 16MB weekly_player_stats or 1.5MB transactions
        const [managersData, matchupsData, standingsData, powerRankingsData, draftData, seasonsMetadataData] = await Promise.all([
        ...
        // Non-blocking background fetch for transactions (~1MB)
        this.transactionsPromise = fetchOrFallback('transactions.json', 'transactions')
            .then(txs => {
        ...
        this.playerStatsPromise = fetchOrFallback('weekly_player_stats.json', 'weekly_player_stats')
            .then(stats => {
        ...
        // Fetch custom league settings, claims, users, and managers from Firebase RTDB with timeout protection
        this.users = {};
        try {
            const [settingsSnap, claimsSnap, managersSnap, conventionSnap, usersSnap, notesSnap, prSnap] = await withTimeout(Promise.all([
                ...
                get(dbRef(database, `leagues/dmsfantasy/commissioner_notes`)).catch(() => null),
                get(dbRef(database, `leagues/dmsfantasy/power_rankings`)).catch(() => null)
            ]), 3500).catch(err => {
                console.warn("RTDB settings timeout/error:", err);
                return [];
            });
            this.preloadedNotes = notesSnap && notesSnap.exists() ? notesSnap.val() : null;
            this.preloadedPR = prSnap && prSnap.exists() ? prSnap.val() : null;
        ...
        const dmsTagline = (this.leagueSettings?.tagline && this.leagueSettings.tagline !== 'In a league of our own')
            ? this.leagueSettings.tagline
            : 'Variance is an excuse for incompetence.';
        const pEl = document.querySelector('.masthead-main p');
        if (pEl) pEl.textContent = dmsTagline;
        ...
        this.notesEngine = new CommissionerNotesEngine({
            ...
            initialData: this.preloadedNotes
        });
    ...
    async renderActiveTab() {
        ...
        if (tab === 'home') {
            ...
            const hasNotesSkeletons = Boolean(document.querySelector('#story .vault-skeleton-notes, #commissioner-note .vault-skeleton-notes'));
    ...
    setupNavigation() {
        ...
        const parseTabFromLocation = () => {
            ...
            if (tabViewMap[tabParam]) return tabParam;
            return 'home';
        };
        ...
        const switchTab = (tab, updateUrl = true) => {
            ...
            // 1. Immediately highlight clicked tab link and activate target view
            if (targetBtn) {
            ...
            // Skeletons only apply to dynamic full-container tabs (transactions, draft, newsletter)
            // Never overwrite static or bespoke containers (home, h2h, records, paradigms, rivalry, admin)
    ...
    renderAdminDashboard() {
        ...
        const leagueSlug = "dmsfantasy";
        const currentTagline = (this.leagueSettings?.tagline && this.leagueSettings.tagline !== 'In a league of our own')
            ? this.leagueSettings.tagline
            : "Variance is an excuse for incompetence.";
        ...
        const buildManagerRow = (m, statusType) => {
            ...
            const leagueUser = (claim?.userId && this.users?.[claim.userId])
                ...
                : (this.users ? Object.values(this.users).find(u => u && typeof u === 'object' && u.managerId === m.id) : null);
            ...
            const statusBadge = statusType === 'active'
                ? `<span style="display: inline-flex; align-items: center; gap: 4px; font-size: 0.68rem; font-weight: 700; padding: 2px 6px; border-radius: 4px; background: #ecfdf5; color: #15803d; border: 1px solid #a7f3d0; margin-left: 4px;"><span style="display: inline-block; width: 5px; height: 5px; border-radius: 50%; background: #16a34a;"></span>Active</span>`
                        ...
                        <div class="admin-actions-cell" style="display: flex; align-items: center; gap: 6px; flex-wrap: wrap;">
                            ${isClaimed ? `
                                <div style="display: inline-flex; flex-direction: column; gap: 2px;">
                                    <span class="badge-registered" style="font-size: 0.72rem; padding: 3px 8px; background: #ecfdf5; color: #065f46; border-radius: 4px; border: 1px solid #a7f3d0; font-weight: 600;" title="Claimed by ${claimEmail}${claim && claim.claimedAt ? ' on ' + new Date(claim.claimedAt).toLocaleDateString() : ''}">
                                        ✓ ${claimEmail}
                                    </span>
        ...
        if (activeMembers.length > 0) {
            managerRows += `
                <tr class="table-group-header" data-status-header="active">
                    <td colspan="3" style="background: var(--bg-surface, #f8fafc); padding: 9px 14px; border-top: 1px solid var(--border-line, #e2e8f0); border-bottom: 1px solid var(--border-line, #e2e8f0);">
        ...
        if (inactiveMembers.length > 0) {
            managerRows += `
                <tr class="table-group-header" data-status-header="inactive">
                    <td colspan="3" style="background: var(--bg-surface, #f8fafc); padding: 9px 14px; border-top: 2px solid var(--border-line, #e2e8f0); border-bottom: 1px solid var(--border-line, #e2e8f0);">
                        <div style="display: flex; align-items: center; justify-content: space-between;">
                ...
                <div id="admin-sec-roster" class="card admin-section-card">
                    ...
                    <div style="margin-top: 1.25rem;">
                        <div class="manager-roster-filters" style="display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 0.85rem; flex-wrap: wrap;">
                            <div style="display: inline-flex; align-items: center; gap: 4px; background: var(--bg-surface, #f1f5f9); padding: 3px; border-radius: 9999px; border: 1px solid var(--border-color, #e2e8f0);">
                                <button type="button" class="btn-roster-filter active" data-filter="all" style="padding: 5px 14px; font-size: 0.78rem; font-weight: 700; border-radius: 9999px; border: none; background: var(--text-primary, #0f172a); color: #fff; cursor: pointer; transition: all 0.15s ease;">
                                    All (${sortedMembers.length})
                                </button>
                                <button type="button" class="btn-roster-filter" data-filter="active" style="padding: 5px 14px; font-size: 0.78rem; font-weight: 700; border-radius: 9999px; border: none; background: transparent; color: var(--text-secondary, #475569); cursor: pointer; transition: all 0.15s ease;">
                                    Active (${activeMembers.length})
                                </button>
                                <button type="button" class="btn-roster-filter" data-filter="inactive" style="padding: 5px 14px; font-size: 0.78rem; font-weight: 700; border-radius: 9999px; border: none; background: transparent; color: var(--text-secondary, #475569); cursor: pointer; transition: all 0.15s ease;">
                                    Inactive (${inactiveMembers.length})
                                </button>
                            </div>
                            <div style="font-size: 0.78rem; color: var(--text-muted, #64748b); font-weight: 500;">
        ...
        container.querySelectorAll('.btn-roster-filter').forEach(pill => {
            pill.addEventListener('click', () => {
                ...
                pill.style.background = 'var(--text-primary, #0f172a)';
