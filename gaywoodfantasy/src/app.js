...
class FantasyApp {
    ...
    async loadData() {
        ...
        try {
            ...
            try {
                const claimsRef = dbRef(database, `leagues/gaywoodfantasy/claims`);
    ...
    renderAdminDashboard() {
                ...
                <div id="admin-sec-identity" class="card admin-section-card">
                    ...
                    <div style="margin-top: 1.25rem;">
                        ...
                        <div class="tagline-input-row">
                            <input type="text" id="admin-league-title-input" class="admin-input" value="${leagueName}" placeholder="e.g. Gaywood / Katz League HQ">
                    ...
                    <div style="margin-top: 1.25rem;">
                        ...
                        <div style="background: var(--bg-surface); border: 1px solid var(--border-color); border-radius: 8px; padding: 1.25rem; margin-bottom: 1.25rem;">
                            <div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap; margin-bottom: 1rem;">
                                ...
                                <div id="admin-loser-active-pill" style="display: inline-flex; align-items: center; gap: 6px; background: rgba(180, 83, 9, 0.1); border: 1px solid rgba(180, 83, 9, 0.35); padding: 4px 10px; border-radius: 6px; font-size: 0.78rem; font-weight: 700; color: #b45309;">
                            ...
                            <div id="admin-loser-current-card" style="background: var(--bg-card); border: 1px solid var(--border-color); border-left: 4px solid var(--accent-gold, #b45309); border-radius: 6px; padding: 1rem 1.25rem;">
                        ...
                        <div style="background: var(--bg-surface); border: 1px solid var(--border-color); border-radius: 8px; padding: 1.25rem;">
                            ...
                            <div style="background: rgba(180, 83, 9, 0.03); border: 1px dashed var(--border-color); border-radius: 6px; padding: 12px 16px; margin-bottom: 1.25rem;">
                                <div style="display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap;">
                                    <div>
        ...
        const updateLoserPreview = () => {
            ...
            if (previewRes && previewRes.manager_id) {
                const mgrName = this.getManagerDisplayName ? this.getManagerDisplayName(previewRes.manager_id, previewRes.manager_name) : (previewRes.manager_name || previewRes.manager_id);
        ...
        const updateLoserUI = (yr) => {
            ...
            if (currentRes && currentRes.manager_id) {
                const curMgr = this.getManagerDisplayName ? this.getManagerDisplayName(currentRes.manager_id, currentRes.manager_name) : (currentRes.manager_name || currentRes.manager_id);
    ...
    async saveLoserCondition(year, ruleConfig) {
        ...
        try {
            const loserRef = dbRef(database, `leagues/gaywoodfantasy/league_settings/loser_conditions/${year}`);
    ...
    async applyLoserConditionToFutureSeasons(ruleConfig) {
        ...
        try {
            const defaultRef = dbRef(database, `leagues/gaywoodfantasy/league_settings/loser_conditions/default`);
    ...
    async applyLoserConditionToAllSeasons(ruleConfig) {
        ...
        try {
            const loserRef = dbRef(database, `leagues/gaywoodfantasy/league_settings/loser_conditions`);
    ...
    showMatchupDetailsModal(homeManagerId, awayManagerId, sNum, wNum) {
        ...
        const leftMgrId = m.home_manager_id || m.team_1_manager_id || hId;
        const rightMgrId = m.away_manager_id || m.team_2_manager_id || aId;
        ...
        const leftName  = m.home_team_name || m.team_1_name || leftMgrName;
        const rightName = m.away_team_name || m.team_2_name || rightMgrName;
        ...
        const roundName = m ? (m.playoff_round || (m.is_playoff ? getPlayoffRoundName(sNum, wNum) : '')) : '';
